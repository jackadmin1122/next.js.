...
> - Currently, `revalidatePath` invalidates all the routes in the [client-side Router Cache](/docs/app/building-your-application/caching#client-side-router-cache). This behavior is temporary and will be updated in the future to apply only to the specific path.
...
**It is required** that your response from a route handler contains `revalidated` field and possible `now` field too, otherwise calling `revalidatePath` will do nothing.
